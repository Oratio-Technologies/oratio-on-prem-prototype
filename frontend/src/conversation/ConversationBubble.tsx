...
import {
  selectChunks,
  selectSelectedDocs,
  selectConversationId, // Add this selector
} from '../preferences/preferenceSlice';
import classes from './ConversationBubble.module.css';
import { FEEDBACK, MESSAGE_TYPE } from './conversationModels';
import SpeakButton from '../components/TextToSpeechButton';
import Refresh from '../assets/renew.svg?react'; // Import Refresh icon
// Import Search icon
import SummarizeIcon from '../assets/summarize.svg?react'; // Import Summarize icon
...
const ConversationBubble = forwardRef<
  HTMLDivElement,
  {
    ...
    conversationId?: string; // Add this prop for flexibility
  }
>(function ConversationBubble(
  {
    ...
    retryBtn,
    onMessageUpdate,
    conversationId: propConversationId, // Accept conversationId prop
  },
  ref,
) {
  ...
  const currentConversationId = useSelector(selectConversationId); // Get current conversation ID from Redux
  ...
  // Use the prop conversationId if provided, otherwise use the one from Redux
  // This adds flexibility to your component
  const conversationId = propConversationId || currentConversationId;
  ...
  // Updated regenerate function with dynamic conversation ID
  const handleRegenerate = async () => {
    ...
    try {
      const response = await conversationService.regenerate({
        conversation_id: '67c99e4011922c087ec4c5b5',
        question: message,
  ...
  // Updated summarize function with dynamic conversation ID
  const handleSummarize = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await conversationService.summarize({
        conversation_id: '67c99e4011922c087ec4c5b5',
        question: message,
      });
  ...
  // Updated expand function with dynamic conversation ID
  const handleExpand = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await conversationService.expand({
  ...
  let bubble;
  if (type === 'QUESTION') {
    // Question bubble code (unchanged)
    bubble = (
      <div
        ref={ref}
      ...
      >
        ...
        <div
          ...
          className="ml-10 mr-2 flex items-center rounded-[28px] bg-green-1000 dark:bg-green py-[14px] px-[19px] text-black dark:text-white max-w-full whitespace-pre-wrap leading-normal"
        >
          {message}
        </div>
      </div>
      ...
      >
        ...
        sources?.some((source) => source.source === 'None') ? null : !sources &&
          ...
          <div className="mb-4 flex flex-col flex-wrap items-start self-start lg:flex-nowrap">
            ...
            <div className="grid grid-cols-2 gap-2 lg:grid-cols-4">
              {Array.from({ length: 4 }).map((_, index) => (
                <div
                  key={index}
                  className="flex h-28 cursor-pointer flex-col items-start gap-1 rounded-[20px] bg-green-1000 p-4 text-green-1000 hover:bg-[#F1F1F1] hover:text-[#6D3ECC] dark:bg-green dark:hover:bg-[#2C2E3C] dark:hover:text-[#8C67D7]"
                >
          ...
          sources && (
            <div className="mb-4 flex flex-col flex-wrap items-start self-start lg:flex-nowrap">
              <div className="my-2 flex flex-row items-center justify-center gap-3">
                <p className="text-[#006955] font-semibold">Sources</p>
              </div>
                ...
                <div className="grid grid-cols-2 gap-2 lg:grid-cols-4">
                  {sources?.slice(0, 3)?.map((source, index) => (
                    <div key={index} className="relative">
                      <div
                        className="h-28 cursor-pointer rounded-[20px] bg-green-1000 p-4 hover:bg-green-1500 dark:bg-green dark:hover:bg-green-3000"
                  ...
                  {(sources?.length ?? 0) > 3 && (
                    <div
                      className="flex h-28 cursor-pointer flex-col-reverse rounded-[20px] bg-green-1000 p-4 text-green hover:bg-[#F1F1F1] hover:text-[#6D3ECC] dark:bg-[#05281C] dark:hover:bg-[#2C2E3C] dark:hover:text-[#8C67D7]"
        ...
        <div className="flex flex-col flex-wrap items-start self-start lg:flex-nowrap">
          <div className="my-2 flex flex-row items-center justify-center gap-3">
            <p className="text-[#006955] font-semibold">Answer</p>
          </div>
          <div
            className={`ml-2 mr-5 flex max-w-[90vw] rounded-[28px] bg-green-1000 py-[14px] px-7 dark:bg-green md:max-w-[70vw] lg:max-w-[50vw] ${
              type === 'ERROR'
        ...
        <div className="my-2 ml-2 flex justify-start">
          ...
          {handleFeedback && (
            <>
              <div
                className={`relative mr-5 flex items-center justify-center lg:invisible 
        ${type !== 'ERROR' ? 'group-hover:lg:visible' : 'hidden'}`}
              >
                <div>
                  <div
                    className={`flex items-center justify-center rounded-full p-2 ${
                      isRefreshHovered
                        ? 'bg-green-1000 dark:bg-green-3000'
                  ...
                  >
                    <Refresh
                      className={`cursor-pointer ${
                        isRefreshHovered
                          ? 'stroke-green dark:stroke-green'
                          : 'stroke-gray-4000'
                      }`}
    ...
    <div className="h-full w-full">
      ...
      <div className="mt-6 flex h-[90%] w-60 flex-col items-center gap-4 overflow-y-auto sm:w-80">
        {sources.sources.map((source, index) => (
          <div
            key={index}
            className="min-h-32 w-full rounded-[20px] bg-green-1000 p-4 dark:bg-green"
          >
