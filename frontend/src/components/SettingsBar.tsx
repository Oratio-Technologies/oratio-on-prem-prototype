...
const useTabs = () => {
  ...
  const tabs = [t('settings.general.label'), t('settings.documents.label')];
  return tabs;
};
    ...
    <div className="relative mt-6 flex flex-row items-center space-x-1 md:space-x-0 overflow-auto">
      ...
      >
        {tabs.map((tab, index) => (
          <button
            key={index}
            onClick={tab === tabs[1] ? undefined : () => setActiveTab(tab)}
            className={`snap-start h-9 rounded-3xl px-4 font-bold hover:text-neutral-600 dark:hover:text-white ${
              activeTab === tab
                ? 'bg-neutral-100 text-neutral-600  dark:bg-green dark:text-green-1000'
                : tab === tabs[1]
                  ? 'text-green-1000 cursor-not-allowed'
                  : 'text-green-1000'
            }`}
            title={tab === tabs[1] ? 'premium feature' : ''}
          >
            {tab}
          </button>
        ))}
